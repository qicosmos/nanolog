...
#if defined (_MSC_VER)
	#include <filesystem>
	namespace nanolog_fs = std::filesystem;
    #if defined max
		#undef max
    #endif
        ...
        namespace nanolog_fs = std::experimental::filesystem;
    #else
        #include <filesystem>
        namespace nanolog_fs = std::filesystem;
    #endif
...
private:
	void init_fileset()
	{
		for(auto& p : nanolog_fs::directory_iterator(m_log_directory))
		{
			const nanolog_fs::path& path = p.path();
...
inline std::atomic<bool>			flush_to_console;
...
public:
	...
	{
		nanolog_fs::create_directories(log_directory);
...
#if defined (NDEBUG)
	#define NANO_LOG(LEVEL) ::waf::nanolog::Logger() == ::waf::nanolog::NanoLogLine(LEVEL, "", __func__, __LINE__)
#else
	#define NANO_LOG(LEVEL) ::waf::nanolog::Logger() == ::waf::nanolog::NanoLogLine(LEVEL, FILE_NAME(__FILE__), __func__, __LINE__)
#endif

#define LOG_INFO ::waf::nanolog::Logger::is_logged(::waf::nanolog::LogLevel::INFO) && NANO_LOG(::waf::nanolog::LogLevel::INFO)
#define LOG_WARN ::waf::nanolog::Logger::is_logged(::waf::nanolog::LogLevel::WARN) && NANO_LOG(::waf::nanolog::LogLevel::WARN)
#define LOG_CRIT ::waf::nanolog::Logger::is_logged(::waf::nanolog::LogLevel::CRIT) && NANO_LOG(::waf::nanolog::LogLevel::CRIT)
